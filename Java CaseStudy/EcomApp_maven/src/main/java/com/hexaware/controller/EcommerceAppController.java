...
public class EcommerceAppController {
    ...
    private static void registerCustomer(OrderProcessorRepositoryImpl orderService, Scanner sc) {
        try {
            ...
            do {
                ...
                System.out.print("Enter customer password (6 characters with alphabets and numerics only): ");
                customerPassword = sc.nextLine();
                attempt++;
            } while (!isValidPassword(customerPassword) && attempt < 3);

            if (!isValidPassword(customerPassword)) {
                System.out.println("Maximum attempts reached. Exiting registration process.");
                System.exit(1);
            }
    ...
    private static void addProduct(OrderProcessorRepositoryImpl orderService, Scanner sc) {
    	System.out.println("Only Employees can handle this session");
        System.out.print("Enter employee ID: ");
        int employeeId = sc.nextInt();
        sc.nextLine();
        if (!orderService.isEmployee(employeeId)) {
    ...
    private static void deleteProduct(OrderProcessorRepositoryImpl orderService, Scanner sc) {
        ...
        sc.nextLine();
        if (!orderService.isEmployee(employeeId)) {
            System.out.println("You are not authorized to delete products.");
            return;
        }
        try {
